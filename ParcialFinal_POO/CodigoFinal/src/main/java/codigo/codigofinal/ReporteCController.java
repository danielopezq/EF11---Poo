...
public class ReporteCController {
    private DatabaseConnection db; //00140323 Aqui se declara una conexion a la base de datos.
    ...
    private void generarReporte(String idCliente) {
        ...
        String query = "DECLARE @ClienteID INT = ?; " + //00140323 Esta es la consulta que se encarga de obtener las tarjetas en el formato que requeria el reporte C de la guia.
                ...
                "    CASE " +
                "        WHEN t.tipo_tarjeta = 'Credito' THEN 'Tarjetas de credito' " +
                "        WHEN t.tipo_tarjeta = 'Debito' THEN 'Tarjetas de debito' " +
                "    END AS TipoTarjeta, " +
        ...
        try (Connection conn = db.getConnection(); //00140323 Sirve para conectarse a la base de datos.
            ...
            while (rs.next()) {
                ...
                if (tipoTarjeta.equals("Tarjetas de credito")) { //00140323 Aca vemos si la tarjeta es de credito.
                    tarjetasCredito.append(numeroTarjeta).append("\n"); //00140323 Se almacena la tarjeta de credito.
                } else if (tipoTarjeta.equals("Tarjetas de debito")) { //00140323 Aca vemos si la tarjeta es de debito.
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(fileName))) { //00140323 Sirve para crear un escritor del archivo de texto del reporte.
            ...
            showAlert("Error", "Ocurrio un error al generar el reporte. Revise los datos ingresados"); //00140323 Se le indica al usuario que hubo un error en la generacion del reporte.
        }
    }
    ...
    public void returnToMainMenu() {
        try {
            Parent root = FXMLLoader.load(getClass().getResource("Menu.fxml")); // 00140323 Aqui se carga el menu principal del archivo fxml.
            Stage stage = new Stage(); // 00140323 Sirve para crear una nueva ventana.
            stage.setTitle("Banco Nacional de Nlogonia"); // 00140323 Aqui se establece el titulo de la ventana.
            stage.setScene(new Scene(root, 1268, 1000)); // 00140323 Aqui definimos las dimensione de la ventana.
            stage.setResizable(false); // 00140323 Aqui se deshabilita la opcion de minimizar o maximizar la ventana.
            stage.show(); // 00140323 Se le muestra la ventana al usuario.
            closeCurrentWindow(); // 00140323 Sirve para cerrar la ventana.
        } catch (IOException e) { // 00140323 Sirve para administrar las excepciones de entrada y salida.
            e.printStackTrace(); // 00140323 Se le muestra la excepcion al usuario.
        }
    }

    @FXML
    public void closeApplication() {
        System.exit(0); // 00140323 Sirve para cerrar el programa.
    }

    private void closeCurrentWindow() {
        Stage stage = (Stage) rootPane.getScene().getWindow(); // 00140323 Sirve para obtener la ventana actual.
        stage.close(); // 00140323 Se cierra la ventana.
    }
